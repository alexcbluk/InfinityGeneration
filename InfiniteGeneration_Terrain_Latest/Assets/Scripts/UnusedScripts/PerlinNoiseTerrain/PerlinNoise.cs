...
public class PerlinNoise : MonoBehaviour {
	...
	private const int GradientSizeTable = 256;
        ...
        public double Noise(double x, double y, double z)
        {
            ...
            int ix = Mathf.FloorToInt((float)x);
            double fx0 = x - ix;
            double fx1 = fx0 - 1;
            double wx = Smooth(fx0);

            int iy = Mathf.FloorToInt((float)y);
            double fy0 = y - iy;
            double fy1 = fy0 - 1;
            double wy = Smooth(fy0);

            int iz = Mathf.FloorToInt((float)z);
            double fz0 = z - iz;
            double fz1 = fz0 - 1;
        ...
        private void InitGradients()
        {
            ...
            {  
                double z = 1f - 2f * Random.value;
                double r = Mathf.Sqrt((float)(1f - z * z));
                double theta = 2 * Mathf.PI * Random.value;
                _gradients[i * 3] = r * Mathf.Cos((float)theta);
                _gradients[i * 3 + 1] = r * Mathf.Sin((float)theta);
                _gradients[i * 3 + 2] = z;
            }
        }
