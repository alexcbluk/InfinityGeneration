...
public class RoadGenerator : MonoBehaviour {
	...
	public static Mesh GenerateRoadSegments(Vector3 [] vertices)
	{
		...
		Vector3 v1, v2 = verticeList[1] - verticeList[0];
		float meshLength1 = v2.magnitude;
		float meshLength = 0;
		Vector3 upright;
		upright = GetUprightVector(v2);
		upright *= halfMeshWidth;
		Vector3[] vertices1 = new Vector3[(verticeList.Count - 1) * 4];
		Vector2[] uv = new Vector2[(verticeList.Count - 1) * 4];
		int[] triangles = new int[6 * (verticeList.Count - 1)];
		triangles[0] = 0;
		triangles[1] = 1;
		triangles[2] = 2;
		triangles[3] = 2;
		triangles[4] = 3;
		triangles[5] = 0;
		Vector3[] normals = new Vector3[(verticeList.Count - 1) * 4];
		for(i = 0; i < vertices1.Length; i++)
		{
			normals[i] = Vector3.up;
		};
		vertices1[0] = verticeList[0] + upright;
		vertices1[1] = verticeList[0] + -upright;
		uv[0] = new Vector2(0, 0);
		uv[1] =	new Vector2(0, meshWidth);
		int index = 2;
		float f = 0;
		float currentU = 0;
		for(i = 0; i < verticeList.Count - 2; i++)
		{
			v1 = -v2;
			v1.Normalize();
			meshLength = meshLength1;
			v2 = verticeList[i + 2] - verticeList[i + 1];
			...
			vertices1[index] = verticeList[i + 1] + -v3;
		...
		vertices1[index] = verticeList[i + 1] - upright;
